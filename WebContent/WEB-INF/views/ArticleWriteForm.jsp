...
<head>
<meta charset="utf-8">
<title>ArticleWriteForm.jsp</title>
  	...
  	<div id="container">
	    <div class="ct_nav">
		      <ul>
				  ...
				  <li class="nav_text"><span><a href="#">글쓰기</a></span></li>
		      </ul>
	    </div><!-- end .ct_nav -->
	    <div class="ct_title">
	      	<h1></h1>
	    ...
	    <div class="notice_main">
	      	<div class="notice_bottom">
	      		<c:choose>
	      			<c:when test="${boardType == 'BT1'}">
	      				<div class="notice_write_title"> 맛집 글쓰기</div>
	      			</c:when>
	      			<c:when test="${boardType == 'BT2'}">
	      				<div class="notice_write_title"> 여행 팁 글쓰기</div>
	      			</c:when>
	      			<c:when test="${boardType == 'BT3'}">
	      				<div class="notice_write_title"> 여행자랑 글쓰기</div>
	      			</c:when>
	      		</c:choose>
				<form method="post" action="articlewrite.action">
					...
					<div class="notie_write_end">
						<button class="notice_write_submit" type="submit" id="submitBtn">등록</button>
						<c:choose>
			      			<c:when test="${boardType == 'BT1'}">
			      				<a href="foodarticleopen.action"><button class="notice_write_cancel" type="button" id="listBtn">목록으로</button></a>
			      			</c:when>
			      			<c:when test="${boardType == 'BT2'}">
			      				<a href="tiparticleopen.action"><button class="notice_write_cancel" type="button" id="listBtn">목록으로</button></a>
			      			</c:when>
			      			<c:when test="${boardType == 'BT3'}">
			      				<a href="storyopen.action"><button class="notice_write_cancel" type="button" id="listBtn">목록으로</button></a>
			      			</c:when>
			      		</c:choose>
