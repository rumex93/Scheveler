	...
	<div id="container">
		<div class="ct_nav">
			<ul>
				...
				<li class="nav_text"><span><a href="#">${memberId } 님의 댓글</a></span></li>
			</ul>
		</div>
		<div class="ct_title">
			<h1></h1>
		...
		<div class="notice_main">
			<div>
				<span href="#" style="color: black; font-size: 30px;">${memberId } 님이 작성한 댓글</span>
				<hr>
			</div>
			<div class="notice_bottom">
				...
				<div class="notice_content">
					<table>
						...
						<tr>
							<td>${comment.boardTypeDetail }</td>
							<c:choose>
								<c:when test="${pageNum == null }">
								<td><a href="javascript:location.href='memberdetailopen.action?articleCode=${comment.articleCode }&bno=${comment.bno }&pageNum=${pageNum = 1 }&memberId=${memberId }'"  style="color: black;">${comment.articleTitle }</a></td>
								</c:when>
								<c:when test="${pageNum != null }">
								<td><a href="javascript:location.href='memberdetailopen.action?articleCode=${comment.articleCode }&bno=${comment.bno }&pageNum=${pageNum }&memberId=${memberId }'"  style="color: black;">${comment.articleTitle }</a></td>
								</c:when>
							</c:choose>
							<td>${comment.memberId }</td>
							<c:choose>
								<c:when test="${pageNum == null }">
								<td><a href="javascript:location.href='memberdetailopen.action?articleCode=${comment.articleCode }&bno=${comment.bno }&pageNum=${pageNum = 1 }&memberId=${memberId }'"  style="color: black;">${comment.articleCommentContent }</a></td>
								</c:when>
								<c:when test="${pageNum != null }">
								<td><a href="javascript:location.href='memberdetailopen.action?articleCode=${comment.articleCode }&bno=${comment.bno }&pageNum=${pageNum }&memberId=${memberId }'"  style="color: black;">${comment.articleCommentContent }</a></td>
								</c:when>
							</c:choose>
