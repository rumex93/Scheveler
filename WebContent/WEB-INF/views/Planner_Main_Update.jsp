...
<body>
		...
		<div id="planner_content">
			...
			<div class="sheveler_leftsidebar">
				<div class="write_schedule">
					<h4>일정 작성</h4>
					<span>${planner.planner_title } : <b>1</b>일
					</span>
					<div class="schedule_calendar">${planner.planner_start_date } ~ ${planner.planner_end_date }</div>
				</div>

				<div class="once_time">
					<ul id="once_time_ul">
					<c:forEach var="day" begin="1" end="${calDateDays }">
						<li data-day="${day }">${day }일차</li>					
					</c:forEach>
					</ul>
				</div>

				<div class="mask"></div>
				<div class="create_planner">일정 수정</div>
				
				
				
				
				<c:forEach var="day" begin="1" end="${calDateDays }">
					<div class="write_schedule_detail" data-daylist="${day }">
						<h4>장소 선택 목록</h4>
						
						
						<c:forEach var="schedule" items="${schedules}">
						<c:if test="${schedule.schedule_is_first eq '1' && schedule.schedule_day eq day}"> <!-- day가 같고 첫번째 장소라면 -->
						<!-- 첫번쨰 숙소 출력 -->
						<div class="destination" data-day="${day }" data-areaval="1" value="${schedule.schedule_value }">
							<input type="hidden" class="x" value="${schedule.schedule_x }">
							<input type="hidden" class="y" value="${schedule.schedule_y }">
							<input type="hidden" class="day" value="${schedule.schedule_day }">
							<input type="hidden" class="isFirst" value="${schedule.schedule_is_first }">
							<div class="schedule_choice_area start-point">
								<span class="dest_name">${schedule.schedule_place }</span> <br>
								<div class="choice_area_content">
									<div class="choice_area_image">
										<img src="${schedule.schedule_img_src }">
									</div>
									<div class="coice_area_time">
										...
										~ 
										종료  <span class="endHr" data-day="${day }">${fn:substring(schedule.schedule_end_time,0,2) }</span>:<span class="endMin" data-day="${day }">${fn:substring(schedule.schedule_end_time,3,5)}</span>
										</p>
										<a class="comment">
											<i class="xi-comment-o xi-x"></i>메모
										</a>
										<div class="memo">
											<i class="xi-close"></i>
											<form>
												<table class="table" id="table">
													<tbody>
														...
														<tr>
															<td colspan="2">
																<textarea class="detailmemo" id="detailmemo" name="detailmemo" rows="4" cols="4">${schedule.schedule_content }</textarea>
															</td>
														</tr>
													</tbody>
												</table><br>
												<button type="button" class="btn save_memo_btn">저장하기</button>
						...
						</c:forEach> <!-- end 첫장소 루프  -->
						
						
						<div class="sortable ui-sortable" data-sortlist="${day }">
						<!-- 그 이후 장소 출력 -->
						<c:forEach var="schedule" items="${schedules}">
						<c:if test="${schedule.schedule_is_first eq '0' && schedule.schedule_day eq day}"> <!-- day가 같고 첫번째 장소가 아니라면 -->
						
						<div class="destination draggable ui-sortable-handle"  value="${schedule.schedule_value }">
						...
						</c:if> <!-- end 첫번째 장소 아니고 day가 같은 경우 -->
						</c:forEach><!-- end schedules 루프 -->
	...
	<script type="text/javascript">
      ...
      */
        ...
        // 수정 폼 최초 요청시 들어있는 장소들 마커 찍기
        updateFormMarker();
        
        // 수정 폼 최초 접근시 검색 비활성화 막기
...
//*--*-*-*-*-*-*-*-*-*-*--*-*--*---*-*-*-*-*-*-*-*-*-*-*-*-*-*
		...
		$('.create_planner').click(function()
		{
			...
			$.ajax({
				type : "post",
				url : "plannerupdateajax.action",
				async : "false",
				data : {schedules : schedule, planner_code : "<%=request.getParameter("planner_code") %>"},
        ...
        function updateFormMarker()
		{
	        ...
	        //------------------------------------------------------------------------------------
	        for (var i = 0; i < xValues.length; i++)  // 마커 찍기
	        {
	            
	            drawNaverMarker(xValues[i].value, yValues[i].value) ;
	        }
	        //-------------------------------------------------------------------------------------------
