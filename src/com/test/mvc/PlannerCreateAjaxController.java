...
public class PlannerCreateAjaxController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		// 1차 페이지 목록 (Planner 테이블)
		String planner_title = (String) session.getAttribute("title");
		String matching_availability_code = (String)session.getAttribute("matching_availability");
		String planner_start_date = (String)session.getAttribute("startdate");
		String planner_end_date= (String)session.getAttribute("enddate");
		String travel_type_code = (String)session.getAttribute("traveltype");
		...
		String transportation_code = (String)session.getAttribute("transportation");
		...
		plannerDTO.setPlanner_budget(budget);
		plannerDTO.setPlanner_min(min);
		...
		try 
		{
			...
			plannerDAO.add(plannerDTO);
			int seq =  plannerDTO.getPlanner_seq();
