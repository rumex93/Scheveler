...
public class StoryDAO implements IArticleDAO
{
	...
	public ArrayList<ArticleDTO> memberCommentLists(int start, int end, String memberId) throws SQLException
	{
		...
		String sql = "SELECT * FROM " + 
				"(" + 
				"    select ROWNUM RNUM, ACV.ARTICLECODE AS ARTICLECODE, ACV.MEMBERID AS MEMBERID, ACV.ARTICLECOMMENTCONTENT AS ARTICLECOMMENTCONTENT,ACV.ARTICLECOMMENTDATE AS ARTICLECOMMENTDATE , AV.BNO AS BNO, AV.BOARDTYPEDETAIL AS BOARDTYPEDETAIL, AV.ARTICLETITLE AS ARTICLETITLE" + 
				"    from ARTICLECOMMENTVIEW ACV LEFT JOIN ARTICLEVIEW AV " + 
				"    ON ACV.ARTICLECODE = AV.ARTICLECODE" + 
				"    where ACV.MEMBERID=?" + 
				") " + 
				"WHERE RNUM>=? AND RNUM<=?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public int getMemberCommentDataCount(String memberId) throws SQLException
	{
		...
		Connection conn = dataSource.getConnection();
		String sql = "select count(*) as count from ARTICLECOMMENTVIEW where MEMBERID=?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, memberId);
	...
	public ArrayList<ArticleDTO> memberLikeLists(int start, int end, String memberId) throws SQLException
	{
		...
		Connection conn = dataSource.getConnection();
		String sql = "SELECT * FROM (SELECT ROWNUM RNUM, TEMP.* FROM(SELECT BOARDTYPEDETAIL ,ARTICLE_LIKES_CODE,M.MEMBER_ID, AL.ARTICLE_CODE ARTICLE_CODE, ARTICLE_LIKES_DATE , BNO, MEMBERID WRITER, ARTICLETITLE, ARTICLELIKE, ARTICLEDATE, ARTICLEHIT FROM ARTICLE_LIKES AL LEFT JOIN ARTICLEVIEW AV ON AL.ARTICLE_CODE = AV.ARTICLECODE LEFT JOIN MEMBER M ON AL.MEMBER_CODE = M.MEMBER_CODE WHERE MEMBER_ID = ? ORDER BY ARTICLE_LIKES_DATE DESC)TEMP )WHERE RNUM >=? AND RNUM<=?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, memberId);
		pstmt.setInt(2, start);
		pstmt.setInt(3, end);
		...
		while (rs.next())
		{
			...
			dto.setBno(rs.getInt("BNO"));							
			dto.setArticleCode(rs.getString("ARTICLE_CODE"));			
			dto.setMemberId(rs.getString("WRITER"));					
			dto.setBoardTypeDetail(rs.getString("BOARDTYPEDETAIL"));	
			dto.setArticleTitle(rs.getString("ARTICLETITLE"));			
			dto.setArticleDate(rs.getString("ARTICLEDATE"));		
			dto.setArticleHit(rs.getInt("ARTICLEHIT"));					
	...
	public int getMemberLikeDataCount(String memberId) throws SQLException
	{
		...
		Connection conn = dataSource.getConnection();
		String sql = "SELECT COUNT(*) AS COUNT " + 
				"FROM ARTICLE_LIKES AL LEFT JOIN ARTICLEVIEW AV " + 
				"ON AL.ARTICLE_CODE = AV.ARTICLECODE " + 
				"LEFT JOIN MEMBER M " + 
				"ON AL.MEMBER_CODE = M.MEMBER_CODE " + 
				"WHERE MEMBER_ID = ? ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, memberId);
